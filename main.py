                ...
                for col in order_columns:
                    ...
                    if not result.empty:
                        ...
                        if row not in st.session_state.scanned:
                            ...
                            st.error("🔴 ДУБЛИКАТ! Этот заказ уже был отсканирован!")
                        found = True
                        break
                if not found:
                    st.error(f"❌ Заказ {search_num} не найден в CSV")
...
if st.session_state.scanned:
    ...
    st.info("Пока нет отсканированных заказов")
...
if st.button("🚀 Сформировать все АПП", type="secondary", use_container_width=True):
    if not st.session_state.scanned:
        ...
        current_date_text = datetime.now().strftime("%d %B %Y").replace('January','января').replace('February','февраля') \
        ...
        try:
            ...
            for code, sheet_name in partner_names.items():
                ...
                # Шапка
                header_lines = headers.get(code, headers['UNKNOWN'])
                current_row = 4
                ...
                for i, row in enumerate(group.itertuples(index=False), 1):
                    ...
                    ws.cell(row=r, column=2, value=str(row_dict.get('Заказ', row_dict.get('заказ', '')))).border = thin_border
                    weight = float(row_dict.get('Вес', row_dict.get('вес', 0)))
                    cost = float(row_dict.get('Цена заказа', row_dict.get('цена заказа', 0)))
                ...
                ws.cell(row=last_row+3, column=1, value="Общее количество отправлений")
                ws.cell(row=last_row+3, column=4, value=len(group))
                
                ws.cell(row=last_row+4, column=1, value="Общий вес отправлений")
                ws.cell(row=last_row+4, column=4, value=round(total_weight, 3))
                
                ws.cell(row=last_row+5, column=1, value="Общая стоимость объявленной ценности")
                ws.cell(row=last_row+5, column=4, value=int(total_cost))
                
                # Подписи
                sign_row = last_row + 8
                
                ws.cell(row=sign_row, column=1, value='Сдал: ООО "ЛЕ МОНЛИД"')
                ws.cell(row=sign_row, column=4, value=f'ПРИНЯЛ: ООО "{sheet_name}"')
                
                ws.cell(row=sign_row+1, column=1, value="Администратор Цепи поставок")
                ws.cell(row=sign_row+1, column=4, value="(должность)")
                
                ws.cell(row=sign_row+3, column=1, value=current_fio)
                ws.cell(row=sign_row+3, column=4, value="_________________")
                
                ws.cell(row=sign_row+4, column=1, value="Ф.И.О.")
                ws.cell(row=sign_row+4, column=4, value="Ф.И.О.")
                
                ws.cell(row=sign_row+6, column=1, value="подпись")
                ws.cell(row=sign_row+6, column=4, value="подпись")
                
                ws.cell(row=sign_row+8, column=1, value="М.П.")
                
                # Границы таблицы
                for r in range(table_start, last_row + 2):
                    for c in range(1, 5):
